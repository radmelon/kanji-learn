...
export async function mnemonicRoutes(server: FastifyInstance) {
    ...
    async (req, reply) => {
      ...
      const mnemonic =
        body.data.model === 'sonnet'
          ? await service.generateSonnet(kanjiId, req.userId!)
          : await service.generateHaiku(kanjiId, req.userId!)
  ...
  server.post<{ Params: { kanjiId: string } }>(
    ...
    async (req, reply) => {
      ...
      const mnemonic = await service.saveUserMnemonic(kanjiId, req.userId!, body.data.storyText)
      return reply.code(201).send({ ok: true, data: mnemonic })
    }
  )
