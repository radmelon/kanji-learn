...
final class APIClient {
    ...
    init(session: URLSession = .shared) {
        ...
        // Note: JSONDecoder is NOT thread-safe for concurrent decode calls.
        // A new instance is created per-request (see request()) rather than
        // sharing one, which prevents EXC_BREAKPOINT crashes when fetchStatus()
        // and fetchQueue() run concurrently on the shared APIClient singleton.
    }
    ...
    private func request<T: Decodable>(
        ...
        do {
            ...
            // thread-safe and triggers EXC_BREAKPOINT on watchOS.
