...
- [x] **(B-212) Hook TTS speaks English only and silently drops the Japanese** — Found on-device in **B144** (owner, 2026-07-27): *"the TTS of the mnemonic was poor quality and skipped any hiragana or Kanji characters that were present in the passage."*
  ...
  **(b) Quality degrades silently to the compact voice.** `getBestVoice` ([tts.ts:17](apps/mobile/src/utils/tts.ts)) returns `undefined` unless an **Enhanced**-quality voice for that language is installed, and `Speech.speak` then falls back to the compact system voice with no signal. On iOS these are a user-initiated download, under **Accessibility → Read & Speak → Voices** (renamed from "Spoken Content" in the current iOS; do not trust older docs, including this line, without checking the device). A device that has never downloaded one always gets the poor voice, with nothing in the app to explain why.

  Owner installed Enhanced English + Japanese voices on 2026-07-27; re-test pending. If quality improves, (b) is a missing OS asset rather than a selection bug — and the code defect reduces to failing silently instead of prompting.

  **(c) An installed voice is never picked up until the app restarts.** `getBestVoice` caches per language in a module-level Map and **caches misses as well as hits** — `cache.set(language, enhanced?.identifier)` stores `undefined`, after which `cache.has(language)` short-circuits every later call. So a learner who follows advice to install a better voice sees no change until they force-quit, and nothing tells them to. This is small but it is the reason (b) is hard to diagnose from inside the app.
...
- [x] **TTS playback drops to barely audible after the speaking leg (speak icons on study cards, browse cards, kun/on rows, vocab, example sentences)** — ROOT-CAUSED 2026-04-23 (commit `f6eb823`): `expo-speech-recognition` flips the iOS `AVAudioSession` to `.playAndRecord` + mode `.measurement` when recognition starts (`ExpoSpeechRecognizer.swift setupAudioSession`) and its `reset()` path never restores it. Every subsequent `Speech.speak()` runs under `.measurement`, which disables system audio processing and attenuates output dramatically. Since the Practice Loop (B134) routes every new/Again/Hard kanji through the speaking leg, the bug fired in virtually every study session.

  **Fix history:** the April fix (shipped in B140 as `7c79eaf`) restored via **expo-av `Audio.setAudioModeAsync` — a native NO-OP in this app**: EXAV only touches the audio session when expo-av itself has active audio objects (`EXAV.m:286`), and all TTS here goes through expo-speech. B140 walkthrough proved the bug persisted. **Real fix (2026-07-04):** restore via expo-speech-recognition's own `setCategoryIOS({category:'playback', mode:'default', categoryOptions:[]})` in the same `end`/`error`/unmount hooks — writes AVAudioSession directly; `mode` must be explicit (the native param defaults to `.measurement`). Corollary: the boot-time `setAudioModeAsync` in `_layout.tsx` is equally a no-op — "cold-kill recovery" was just the fresh process's default session.
