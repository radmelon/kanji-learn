...
- [ ] **Pitch Accent Indicator** — Display the pitch accent pattern (高低 pattern) for kun-yomi readings on the KanjiCard. Sourced from an open pitch accent dictionary (e.g., Wadoku or a bundled dataset). Particularly valuable for intermediate learners targeting natural spoken Japanese. **Note 2026-04-19:** pitch accent is fundamentally a word-level property (`橋` LH vs `箸` HL — same reading, opposite pitch) — annotating isolated kanji readings is lower-value than annotating the vocab entries. Scope this alongside E5 and the vocab-level voice drill umbrella (see "Vocab as the drill unit" Build 3-C work).
...
- [ ] **Expand Example Vocab + Sentences per Kanji (Source from JMdict / Kanjidic / Tatoeba)** — Today each kanji has just 2 example vocab entries and 2 example sentences in the DB (confirmed via SELECT on 息, id=1599, 2026-04-19). For deeper study value — and because some entries don't even contain the kanji itself (see BUGS.md entry for 息 paired with 呼吸) — expand each kanji's `example_vocab` to ~5–10 entries and `example_sentences` to ~3–5. Sources: (1) JMdict (XML dump) gives comprehensive vocabulary with readings + glosses, filterable to words containing the target kanji; (2) Tatoeba bulk corpus already used for sentences (see `packages/db/src/seeds/seed-sentences.ts`) — rerun with a higher per-kanji cap; (3) Kanjidic2 gives per-character meta but not vocab. Implementation: extend the seed pipeline to emit more entries per row, add a validator (from the kanji-doesn't-contain-itself bug) to reject entries where `word` lacks the target character, and keep the DB schema (already jsonb arrays, no migration). UI will show more vocab/sentences with no code change on the consumer side — it's already a `.map()` over the array.
  `[Effort: M]` `[Impact: High]` `[Backend: Yes — seed pipeline + data regen]` `[Status: 💡 Idea]`
