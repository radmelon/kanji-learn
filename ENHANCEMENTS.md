...
- [ ] **Weighted Confidence Scoring (Easy=3 / Good=2 / Hard=1 / Again=0)** — Implemented in B121 (server commit `aaa874a`, client commit `dededf3`, App Runner deploy op `7a2c8a31df514442bedbc29b0c79ab8a` on 2026-04-18). Code ships in B121 TestFlight. **Math correctness not yet verified** — user has confirmed the "confidence" label but not that the displayed % uses the new weighted formula. Ground truth revealed no data migration was needed — `review_logs.quality` was already stored 0–5, so the weighted formula applies retroactively to all historical reviews. **Sanity test to close:** grade 1 Easy + 1 Hard in a 2-card session → expected weighted result `(3+1)/(2×3) = 67%`. Binary would show `50%`.
  `[Effort: M]` `[Impact: High]` `[Backend: Yes]` `[Status: 🧪 Code shipped; math verification pending]`
